...
class DCRNNSupervisor:
    ...
    def save_model(self, epoch):
        ...
        config['epoch'] = epoch
        torch.save(config, os.path.join(savepath,'epo%d.tar' % epoch))
        self._logger.info("Saved model at {}".format(epoch))
        return os.path.join(savepath,'epo%d.tar' % epoch)

    def load_model(self):
        self._setup_graph()
        assert os.path.exists(os.path.join(savepath,'epo%d.tar' % self._epoch_num)), \
            'Weights at epoch %d not found' % self._epoch_num
        checkpoint = torch.load(os.path.join(savepath,'epo%d.tar' % self._epoch_num), map_location='cpu')
    ...
    def _train(self, base_lr,
        ...
        wait = 0
        optimizer = torch.optim.Adam(self.dcrnn_model.parameters(), lr=base_lr, eps=epsilon,weight_decay=1e-3)
        ...
        for epoch_num in range(self._epoch_num, epochs):
            ...
            for _, (x, y) in enumerate(train_iterator):
                ...
                batches_seen += 1
                loss.backward()
                # print(list(self.dcrnn_model.parameters()))
    ...
    def _compute_loss(self, y_true, y_predicted):
        ...
        # print(y_predicted.size(), y_true.size())
    ...
    def evaluate_acc(self, dataset='test', batches_seen=0):
        with torch.no_grad():
            ...
            y_truth_scaled, y_pred_scaled = self.standard_scaler.inverse_transform(y_truth), self.standard_scaler.inverse_transform(y_pred)
            ...
            for step in range(self.horizon):
                ...
                for i in range(y_truth_cls.size):
                    if y_truth_scaled_step[i] < 2/3:
                        y_truth_cls[i] = 0
                        low_idx.append(i)
                    elif y_truth_scaled_step[i] >= 4 / 3:
                        y_truth_cls[i] = 2
                        high_idx.append(i)
                    else:
                        y_truth_cls[i] = 1
                ...
                for i in range(y_pred_cls.size):
                    if y_pred_scaled_step[i] < 2/3:
                        ...
                        y_pred_cls[i] = 2
                # print(y_truth_cls.size,y_truth_cls[high_idx].size,y_truth_cls[normal_idx].size,y_truth_cls[low_idx].size)
                acc = sum(y_truth_cls==y_pred_cls)/(y_truth_cls.size)
                accH = sum(y_truth_cls[high_idx]==y_pred_cls[high_idx])/(y_truth_cls[high_idx].size) if y_truth_cls.size else 0
                accN = sum(y_truth_cls[normal_idx]==y_pred_cls[normal_idx])/(y_truth_cls[normal_idx].size) if normal_idx else 0
                accL = sum(y_truth_cls[low_idx]==y_pred_cls[low_idx])/(y_truth_cls[low_idx].size)if low_idx else 0
...
# classn=5
# yyy=torch.randn(1,120,126,1, classn, requires_grad=True)
# input = torch.randn(1,120,126, classn, requires_grad=True)
# target = torch.empty(1,120,126, dtype=torch.long).random_(classn)
