...
parser.add_argument('--data',type=str,default='/devdata/zhaohaoran/airspacedata',help='data path')
parser.add_argument('--adjdata',type=str,default='/devdata/zhaohaoran/adj_mx_geo_126.csv',help='adj data path')
...
parser.add_argument('--checkpoint', type=str, default='save_exp1_best_0.18.pth', help='')
...
def main():
    ...
    print(truth.size(), preds.size(),y_hat.size())
    acc_mean, pred_all, truth_all = [], [], []
    for i in range(args.seq_length):
        pred_i = scaler.inverse_transform(y_hat[:,:,i]).cpu().detach().numpy()
        truth_i=scaler.inverse_transform(truth[:,:,i]).cpu().detach().numpy()
